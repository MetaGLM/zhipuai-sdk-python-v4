...
from zhipuai.core._http_client import ZHIPUAI_DEFAULT_TIMEOUT, make_user_request_input
...
from zhipuai.core._request_opt import ClientRequestParam
...
def _get_params(client: HttpClient[Any, Any]) -> dict[str, str]:
    request = client._prepare_request(ClientRequestParam(method="get", url="/foo"))
...
class TestZhipuAI:
    ...
    def test_request_timeout(self) -> None:
        request = self.client._prepare_request(ClientRequestParam(method="get", url="/foo"))
        timeout = httpx.Timeout(**request.extensions["timeout"])  # type: ignore
        assert timeout == ZHIPUAI_DEFAULT_TIMEOUT

        request = self.client._prepare_request(
            ClientRequestParam(method="get", url="/foo", timeout=httpx.Timeout(100.0))
        )
        timeout = httpx.Timeout(**request.extensions["timeout"])  # type: ignore
    ...
    def test_default_headers_option(self) -> None:
        ...
        request = client2._prepare_request(ClientRequestParam(method="get", url="/foo"))
    ...
    def test_validate_headers(self) -> None:
        ...
        assert request.headers.get("Authorization") == generate_token(api_key)
    ...
    def test_request_extra_headers(self) -> None:
        request = self.client._prepare_request(
            ClientRequestParam(
                method="post",
                url="/foo",
                **make_user_request_input(extra_headers={"X-Foo": "Foo"}),
            ),
        )
    ...
    def test_get(self, respx_mock: MockRouter) -> None:
        ...
        response = self.client.get("/foo", cast_type=dict)
        ...
        response = self.client.get("/boo", cast_type=dict)
        assert response == '{"result": "success"}'
    ...
    def test_post(self, respx_mock: MockRouter) -> None:
        ...
        response = self.client.post("/foo", cast_type=dict, body={"foo": "bar"})
        assert response == {"result": "success"}
    ...
    def test_patch(self, respx_mock: MockRouter) -> None:
        ...
        response = self.client.patch("/foo", cast_type=dict, body={"foo": "bar"})
        assert response == {"result": "success"}
    ...
    def test_delete(self, respx_mock: MockRouter) -> None:
        ...
        response = self.client.delete("/foo", cast_type=dict, body={"foo": "bar"})
        assert response == {"result": "success"}
